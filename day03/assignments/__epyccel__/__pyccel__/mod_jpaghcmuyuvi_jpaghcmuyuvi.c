#include "mod_jpaghcmuyuvi_jpaghcmuyuvi.h"
#include <stdlib.h>
#include "ndarrays.h"
#include <stdint.h>
#include <math.h>
...
int jacobi(t_ndarray a, t_ndarray b, t_ndarray x, int64_t n, double eps, double *stand, int64_t *iteration)
{
    ...
    while((1 != 0))
    {
        ...
        #pragma omp parallel
        {
            #pragma omp for collapse(2) reduction(*: x_courant) reduction(+: x_courant)
            for (i = 0; i < n; i += 1)
            {
                ...
                #pragma omp for reduction(*: x_courant) reduction(+: x_courant) reduction(/: x_courant)
                for (j = i + 1; j < n; j += 1)
                {
                    GET_ELEMENT(x_courant, nd_double, (int64_t)i) += GET_ELEMENT(a, nd_double, (int64_t)j, (int64_t)i) * GET_ELEMENT(x, nd_double, (int64_t)j);
                }
